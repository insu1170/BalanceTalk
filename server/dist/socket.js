...
const initSocket = (server) => {
    io = new socket_io_1.Server(server, {
        path: "/socket.io",
        cors: {
            origin: ["http://localhost:3000", "https://balance-talk-three.vercel.app"],
            methods: ["GET", "POST"],
            credentials: true,
        },
    ...
    io.on("connection", (socket) => {
        ...
        socket.on("join_room", (data) => {
            ...
            const result = (0, rooms_1.joinRoom)(roomId, userId, name);
            if (!result.success) {
        ...
        socket.on("send_message", (data) => {
            ...
            // 유저의 진영 정보 가져오기
            const side = (0, rooms_1.getUserSide)(data.roomId, data.userId);
        ...
        socket.on("start_debate", (data) => {
            ...
            // 방장 권한 확인
            const room = (0, rooms_1.getRoom)(data.roomId);
            if (!room)
                return;
            ...
            (0, rooms_1.startDebate)(data.roomId, data.topic);
            ...
            setTimeout(() => {
                ...
                const updatedRoom = (0, rooms_1.startMainDebate)(data.roomId, 10000);
                if (updatedRoom) {
                    io.to(data.roomId).emit("debate_progress", {
                        phase: 'debating',
                    ...
                    setTimeout(() => {
                        ...
                        const finalRoom = (0, rooms_1.startFinalSelection)(data.roomId);
                        if (finalRoom) {
                            io.to(data.roomId).emit("debate_progress", {
                            ...
                            setTimeout(() => {
                                ...
                                const resetRoom = (0, rooms_1.endDebate)(data.roomId);
                                if (resetRoom) {
                                    io.to(data.roomId).emit("debate_progress", {
                                        phase: 'waiting',
                                        endTime: 0,
        ...
        socket.on("select_side", (data) => {
            ...
            const success = (0, rooms_1.selectSide)(data.roomId, data.userId, data.side);
            if (success) {
                // 실시간 선택 현황 브로드캐스트
        ...
        socket.on("disconnect", () => {
            if (currentRoomId && currentUserId) {
                ...
                const room = (0, rooms_1.getRoom)(currentRoomId);
                if (!room)
                    return;
                ...
                const timer = setTimeout(() => {
                    ...
                    const updatedRoom = (0, rooms_1.leaveRoom)(currentRoomId, currentUserId); // ! checks are safe due to closure
                    if (updatedRoom) {
                        const hostId = Object.keys(updatedRoom.users)[0];
